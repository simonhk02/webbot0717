...
class EventHandlers {
  ...
  setupMessageHandlers() {
    ...
    eventBus.on(EventTypes.MESSAGE.RECEIVED, async (event) => {
      ...
      // ====== 新增：白名單群組判斷 ======
      const { GROUP_WHITELIST, GROUP_NAME_MAP } = require('../config/whitelist');
      if (chatId && GROUP_WHITELIST.length > 0 && !GROUP_WHITELIST.includes(chatId)) {
        // 非白名單群組，回覆提示訊息
        const groupName = GROUP_NAME_MAP[GROUP_WHITELIST[0]] || '指定群組';
        const reply = `請在正確的群組 "${groupName}" 中使用此功能\n\n當前群組: ${chatId}`;
        try {
          await whatsAppService.sendMessage(chatId, reply);
        } catch (e) {
          businessLogger.warn('白名單提示訊息發送失敗', e);
        }
        return; // 終止後續處理
      }
      // ====== 白名單判斷結束 ======
