...
class FixedComprehensiveSystemTest {
    constructor() {
        this.results = [];
    ...
    async testSystemHealth() {
        ...
        try {
            ...
            if (response.statusCode === 200) {
                ...
                this.log('工作流系統健康檢查', 'PASS', `系統健康，狀態: ${data.status}`);
            } else {
                this.log('工作流系統健康檢查', 'FAIL', `健康檢查返回狀態碼${response.statusCode}`);
            }
        } catch (error) {
            this.log('工作流系統健康檢查', 'FAIL', `健康檢查失敗`, error.message);
        }
        ...
        try {
            ...
            await this.makeRequest(this.workflowPort, '/workflow/health');
    ...
    async testAPIEndpoints() {
        ...
        const endpoints = [
            { path: '/workflow/health', method: 'GET', expectedStatus: 200 },
        ...
        for (const file of frontendFiles) {
            try {
                if (fs.existsSync(file)) {
                    ...
                    // 檢查是否包含N8N風格的關鍵元素
                    const hasN8NStyle = content.includes('workflow-designer') || 
                                       content.includes('node-') || 
                                       content.includes('drag');
                    
                    if (hasN8NStyle) {
                        this.log(`前端文件 ${file}`, 'PASS', `文件存在且包含N8N風格元素，大小${stats.size}字節`);
                    } else {
        ...
        for (const dbFile of dbFiles) {
            try {
                if (fs.existsSync(dbFile)) {
                    ...
                    this.log(`數據庫文件 ${dbFile}`, 'SKIP', '文件不存在，可能使用內存數據庫');
                }
            } catch (error) {
    ...
    async testBotManagement() {
        ...
        try {
            const testBot = {
                name: 'TestBot_Management_' + Date.now(),
                type: 'whatsapp',
                purpose: 'management testing',
                aiLevel: 'advanced',
            ...
            if (createResponse.statusCode === 200 || createResponse.statusCode === 201) {
                const createdBot = JSON.parse(createResponse.body);
                this.log('機械人創建功能', 'PASS', `成功創建機械人: ${testBot.name}`);
                ...
                if (listResponse.statusCode === 200) {
                    ...
                    const foundBot = listData.bots && listData.bots.find(bot => bot.name === testBot.name);
                    if (foundBot) {
                        this.log('機械人列表更新', 'PASS', '新創建的機械人出現在列表中');
                    } else {
                        this.log('機械人列表更新', 'FAIL', '新創建的機械人未出現在列表中');
                    }
                }
            } else {
                this.log('機械人創建功能', 'FAIL', `創建失敗，狀態碼${createResponse.statusCode}`);
            }
        } catch (error) {
            this.log('機械人創建功能', 'FAIL', '創建測試失敗', error.message);
        }
    }
    ...
    async testPerformance() {
        ...
        try {
            ...
            if (response.statusCode === 200 && responseTime < 100) {
                this.log('單請求響應時間', 'PASS', `響應時間${responseTime}ms，性能優秀`);
            } else if (response.statusCode === 200 && responseTime < 500) {
                this.log('單請求響應時間', 'PASS', `響應時間${responseTime}ms，性能良好`);
            } else {
                this.log('單請求響應時間', 'FAIL', `響應時間${responseTime}ms，性能較慢`);
            }
        } catch (error) {
            this.log('單請求響應時間', 'FAIL', '響應時間測試失敗', error.message);
        }
    }
    ...
    generateReport() {
        ...
        if (successRate >= 90) {
            ...
            console.log(`\n⚠️ 系統狀態: 一般 (${successRate}%)`);
        } else {
            console.log(`\n❌ 系統狀態: 需要改進 (${successRate}%)`);
        }
        ...
        try {
            ...
            console.log(`\n📝 詳細報告已保存到: test-report-fixed.json`);
        } catch (error) {
...
// 執行測試
async function main() {
    const tester = new FixedComprehensiveSystemTest();
...
module.exports = FixedComprehensiveSystemTest; 